...
class QueryTests(TestCase):
    ...
    def test_searchTerms(self):
        ...
        q.searchTerms = 'zx81'
        self.assertTrue(q.url() == \
                'http://www.koders.com/?s=zx81&output=rss')

    def test_count(self):
        q = Query(QueryTests.template)
        q.count = 25
        self.assertTrue(q.url() == \
            'http://www.koders.com/?c=25&output=rss')

    def test_startPage(self):
        q = Query(QueryTests.template)
        q.startPage = 5
        self.assertTrue(q.url() == \
            'http://www.koders.com/?p=5&output=rss')

    def test_startIndex(self):
        q = Query(QueryTests.template)
        q.startIndex = 5
        self.assertTrue(q.url() == \
            'http://www.koders.com/?i=5&output=rss')
